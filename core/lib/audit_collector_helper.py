...
class AuditHelpers(object):
    ...
    def __init__(self, 
        ...
        self.server_host="11.11.11.2"
        self.remote_user="cisco"
    ...
    def send_to_server(self, filename=None, remote_dest=None):
        if filename is None:
            self.syslogger.info("No filename specified, bailing out")
            return 1

        if remote_dest is None:
            self.syslogger.info("No  remote destination on server specified, bailing out")
            return 1

        cmd =  "cat "+filename+"| ssh –i "+self.id_rsa_file
        cmd =  cmd + "-o StrictHostKeyChecking=no "
        cmd =  cmd + self.remote_user+"\@"+self.server_host+" \"cat > "+remote_dest+"\""
    ...
    def validate_and_send(self):
        ...
        for domain in ["XR-LXC", "ADMIN-LXC", "HOST"]:
            local_file = "/misc/app_host/"+domain+".xml"
            self.send_to_server(filename=local_file,
                                remote_dest= "~/"+domain+".xml")
